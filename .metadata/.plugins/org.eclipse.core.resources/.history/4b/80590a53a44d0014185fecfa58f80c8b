...
p.addSkillXP(xpamt*3*p.skillLvl[6], 6);
return false;
}
...
if(ps.absX < p2.absX+3 && ps.absY < p2.absY+3 && ps.absX > p2.absX-3 && ps.absY > p2.absY-3 && ps.playerId != p.playerId && ps.playerId != p2.playerId) 
{
                            gfx(ps, 369, 0);                      
        ...
        sendMsg(p,"You dont have the required runes for this spell.");
           ...
           case 7: // BLood Barrage
			...
			deleteItems(p, death, 4, soul, 1, blood, 6);
			    p2.appendHit(hitdmg, 0);
				for(Player ps : Engine.players) {
					if(ps != null) {

if(ps.absX < p2.absX+3 && ps.absY < p2.absY+3 && ps.absX > p2.absX-3 && ps.absY > p2.absY-3 && ps.playerId != p2.playerId && ps.playerId != p.playerId) 
{
                            gfx(ps, 377, 0);                      
                            ps.appendHit(hitdmg, 0);
			    p.updateHP(hitdmg / 4, true);	                         
...
case 15: // Shadow Barrage
                if (p.magicDelay <= 0) {
                    ...
                    if (magicLvl(p) >= ancientLvlReq[p.clickId]) {
                        ...
                        if (hasReq(p, death, 4) && hasReq(p, air, 4) && hasReq(p, blood, 2) && hasReq(p, soul, 3)) {
			    ...
			    p.combatDelay += p.attackDelay;
			    gfx(p2, 383, 0);
			deleteItems(p, death, 4, soul, 3, blood, 2, air, 4);
			    p2.appendHit(hitdmg, 0);
				for(Player ps : Engine.players) {
					if(ps != null) {
