...
public class PlayerOption1 implements Packet {
	...
	public void handlePacket(Player p, int packetId, int packetSize) {
		...
		if (p2 != null & p != null) {
			if (!Engine.wildernessArea(p.absX, p.absY) && p.AtDuel() == false && p.AtClanField() == false && p.AtCastleWars() == false && !Engine.wildernessArea(p2.absX, p2.absY) && p2.AtDuel() == false && p2.AtClanField() == false && p2.AtCastleWars() == false) {
				return;
			}
