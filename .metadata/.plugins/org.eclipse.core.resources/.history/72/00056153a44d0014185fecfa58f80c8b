...
public class PlayerNPCCombat {
	...
	public void attackNPC(Player p) {
		...
		if ((p != null) && (n != null)) {
			...
			if (p.magicNPC.autoCasting) {
				...
				if (Misc.getDistance(n.absX, n.absY, p.absX, p.absY) <= 10) {
					...
					if (p.equipment[3] == 4214) {
						...
						// p.frames.createGlobalProjectile(p.absY, p.absX,
						// offsetY, offsetX,
						...
						p.frames.sendMessage(p, "You need some arrows to attack a monster!");
						resetAttack(p);
					}
			...
			} else if (Misc.getDistance(n.absX, n.absY, p.absX, p.absY) <= 1 && UsingABow(p.equipment[3]) == false) {
				if (!p.usingSpecial) {
					p.requestAnim(p.attackEmote, 0);
				} else if (p.usingSpecial) {
					p.frames.setConfig(p, 301, 0);
					if (p.equipment[3] == 11694 && p.specialAmount >= 50) { // Armadyl
						...
						p.requestGFX(1221, 0);
					} else if (p.equipment[3] == 3101 && p.specialAmount >= 50) { // Dragon
																					// Claws.
						hitDamage = Misc.random((int) (maxMeleeHit(p) * 1.1));
						p.usingSpecial = false;
