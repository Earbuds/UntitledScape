...
import untitledscape.io.packets.Prayer;
...
public class ActionButtons implements Packet {
	...
	public void handlePacket(Player p, int packetId, int packetSize) {
		...
		case 274:
			if (p.donecode == 1) {
				switch (buttonId) {
				case 5: // Legends Quest
					if (p.legendsQuest == 0) {
						p.frames.showInterface(p, 275);
						p.frames.setString(p, "Legends Quest", 275, 2);
						p.frames.setString(p, "", 275, 11);
						p.frames.setString(p, "To start this quest, speak to a Legends Guard.", 275, 12);
						p.frames.setString(p, "They are located outside the Legends Guild.", 275, 13);
						p.frames.setString(p, "Teleport to Ardougne and follow the West path.", 275, 14);
						if (true) {
							p.frames.setString(p, "<col=00AA00>You meet the requirements for this quest!", 275, 15);
						} else {
							p.frames.setString(p, "<col=BB0000>You don't meet the requirements for this quest.", 275, 15);
						}
						for (int i = 0; i < 100; i++)
					...
					if (p.legendsQuest == 1) {
						...
						p.frames.setString(p, "Legends Quest", 275, 2);
						p.frames.setString(p, "", 275, 11);
						p.frames.setString(p, "I have just accepted the hardest quest in all of DavidScape.", 275, 12);
						p.frames.setString(p, "It turns out their leader has been kidnapped!", 275, 13);
						p.frames.setString(p, "I need to speak to a Legends Guard for more information.", 275, 14);
						for (int i = 0; i < 100; i++)
					...
					if (p.legendsQuest == 2) {
						...
						p.frames.setString(p, "Legends Quest", 275, 2);
						p.frames.setString(p, "", 275, 11);
						p.frames.setString(p, "The Legends Guard said the last person to see the leader was", 275, 12);
						p.frames.setString(p, "<col=0000BB>Malignius Mortifer<col=000000>. He is South of Falador.", 275, 13);
						for (int i = 0; i < 100; i++)
					...
					if (p.legendsQuest == 3) {
						...
						p.frames.setString(p, "Legends Quest", 275, 2);
						p.frames.setString(p, "", 275, 11);
						p.frames.setString(p, "<col=0000BB>Malignius Mortifer<col=000000> wont talk to me.", 275, 12);
						p.frames.setString(p, "I need to talk to the Legends Guard again.", 275, 13);
						for (int i = 0; i < 100; i++)
							p.frames.setString(p, "", 275, 14 + i);
					}
					if (p.legendsQuest == 4) {
						...
						p.frames.setString(p, "", 275, 11);
						p.frames.setString(p, "<col=0000BB>Malignius Mortifer<col=000000> wont talk to me.", 275, 12);
						p.frames.setString(p, "The Legends Guard told me to talk to the Woodcutting Tutor.", 275, 13);
						for (int i = 0; i < 100; i++)
							p.frames.setString(p, "", 275, 14 + i);
					}
					if (p.legendsQuest == 5) {
						...
						p.frames.setString(p, "Legends Quest", 275, 2);
						p.frames.setString(p, "", 275, 11);
						p.frames.setString(p, "The <col=0000BB>Woodcutting Tutor<col=000000> told me about the Dramen tree.", 275, 12);
						p.frames.setString(p, "Once I cut a branch, I can fletch it into a staff.", 275, 13);
						p.frames.setString(p, "There is a portal North of the Woodcutting area to take me there.", 275, 14);
						for (int i = 0; i < 100; i++)
							p.frames.setString(p, "", 275, 15 + i);
					}
					break;
				...
				case 16:// Membership/Donate
					p.frames.sendMessage(p, "www.DavidScape.com/mem.html");
					p.frames.sendMessage(p, "Client v4.3 and up has the link in the Links tab.");
					break;
				case 13:// Assault
				...
				case 17:// Apply Staff
					if (p.rights >= 1) {
						p.frames.sendMessage(p, "Your already staff!");
					} else {
						p.frames.sendMessage(p, "www.DavidScape.com/smf");
						p.frames.sendMessage(p, "Client v4.3 and up has the link in the Menu.");
					}
					break;
				case 18:// Client
					p.frames.sendMessage(p, "www.DavidScape.com/");
					p.frames.sendMessage(p, "Client v4.3 and up has the link in the Menu.");
		...
		case 336:
			...
			p.frames.sendMessage(p, "" + buttonId);
			p.frames.sendMessage(p, "" + buttonId2);
			p.frames.sendMessage(p, "" + interfaceId);
			p.frames.sendMessage(p, "" + packetId);
			break;
		case 271:
		...
		case 75: // Dragon baxe special
			if (buttonId == 10 && !p.usingSpecial && p.equipment[3] == 1377 && p.specialAmount >= 100) {
				p.specialAmount -= 100;
			...
			} else if (buttonId == 10 && !p.usingSpecial && p.equipment[3] == 1377 && p.specialAmount <= 99) {
		...
		case 762:
			...
			} else if (buttonId == 41 || buttonId == 39 || buttonId == 37 || buttonId == 35 || buttonId == 33 || buttonId == 31 || buttonId == 29 || buttonId == 27 || buttonId == 25) { // Tab
																																															// buttons
				if (packetId == 21) { // Collapse
					Engine.playerBank.collapseTab(p, Engine.playerBank.getArrayIndex(buttonId)); // This
																									// method
																									// will
																									// be
																									// added
