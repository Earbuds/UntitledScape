		...
		if (interfaceId == 149) {
			...
			case 11256: // dragon imp
				...
				if (Misc.random(100) <= 1) {
					pi.deleteItem(p, 11256, pi.getItemSlot(p, 11256), 1);
					pi.addItem(p, p.DragonImpVeryRare(), 1);
					p.frames.sendMessage(p, "You open the jar and find a <shad=1569541>VERY RARE </col>item.");
					p.ImpLuck = 0;
				} else if (Misc.random(10) <= 1) {
					pi.deleteItem(p, 11256, pi.getItemSlot(p, 11256), 1);
					pi.addItem(p, p.DragonImpRare(), 1);
					p.frames.sendMessage(p, "You open the jar and find a <shad=15695415>RARE </col>item.");
					p.ImpLuck = 0;
				} else {
					pi.deleteItem(p, 11256, pi.getItemSlot(p, 11256), 1);
					pi.addItem(p, p.DragonImpCommon(), 1);
					p.frames.sendMessage(p, "You open the jar and find a <col=1532693>COMMON </col>item.");
					p.ImpLuck -= 2;
				}
			...
			case 9003:
				...
				//p.frames.setString(p, "Forums are: DavidScape.com!", 156, 8);
				...
				p.frames.setString(p, "~Owner is: Earbuds~ -Co-Owner is: E Z Range-", 156, 14);
				p.frames.setString(p, "Have fun on UntitledScape 508!", 156, 15);
				for (int i = 1; i < 82; i++) {
					p.frames.setString(p, "~~~~~UntitledScape~~~~~", 156, 15 + i);
				}
				break;
