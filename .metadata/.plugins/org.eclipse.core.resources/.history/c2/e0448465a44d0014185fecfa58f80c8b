...
public class NPCOption1 implements Packet {
	...
	public static void GetSkillCape(Player p, int Dialogue, int Npc, String Name, String Skill) {
		p.Dialogue = Dialogue;
	...
	public static void OnlyLevel(Player p, int Dialogue, int Npc, String Name, int Skill, String Nskill) {
		p.Dialogue = Dialogue;
		p.frames.showChatboxInterface(p, 241);
	...
	public void handlePacket(Player p, int packetId, int packetSize) {
		...
		if (pnp.npcType == 1835) {
			if ((p.rights >= 1) || (p.member > 0)) {
				...
				p.frames.setString(p, "You cant buy here! Your not a member!", 241, 4);
			}
		}
		if (pnp.npcType == 5029) {
			if (p.rewards < 1) {
		...
		if (pnp.npcType == 398) {
			if (p.legendsQuest == 0) {
				LegendsQuest.startQuest(p);
			} else if (p.legendsQuest == 1) {
				LegendsQuest.partTwo(p);
			} else if (p.legendsQuest == 2) {
				LegendsQuest.retell(p);
			} else if (p.legendsQuest == 3) {
				LegendsQuest.heWontTalk(p);
			} else if (p.legendsQuest < 7) {
				LegendsQuest.retell2(p);
			}
		}
		if (pnp.npcType == 2713) {
			if (p.legendsQuest < 2) {
				p.frames.sendMessage(p, "He only talks to Legends.");
			} else if (p.legendsQuest == 2) {
				LegendsQuest.maligniusScared(p);
			} else if (p.legendsQuest < 7) {
		...
		// Implings
		if (pnp.npcType == 6055) { // Baby Impling
			Implings.catchImpling(p, 1, 125, 11238, "baby");
		}
		
		if (pnp.npcType == 6056) { // Young Impling
			Implings.catchImpling(p, 40, 300, 11240, "young");
		}
		
		if (pnp.npcType == 6062) { // Magpie Impling
			Implings.catchImpling(p, 60, 600, 11252, "magpie");
		}
		
		if (pnp.npcType == 6063) { // Ninja Impling
			Implings.catchImpling(p, 70, 800, 11254, "ninja");
		}
		
		if (pnp.npcType == 6064) { // Dragon Impling
			Implings.catchImpling(p, 83, 1250, 11256, "dragon");
		}
		...
		if (pnp.npcType == 8023) {
			if (p.rights >= 1) {
				...
				p.frames.setString(p, "Runes too OP, have some dank kush instead m80.", 241, 4);
				Engine.playerItems.addItem(p, 6056, 420);
			} else if (p.rights <= 0) {
				p.frames.showChatboxInterface(p, 241);
				p.frames.animateInterfaceId(p, 9850, 241, 2);
				...
				p.frames.setString(p, "Yellow Orb", 241, 3);
				p.teleportTo(3222, 3219, 0, 0, 0, 8939, 8941, 1576, 0, 1577, 0);
				p.frames.setString(p, "What are you doing up here! You're not staff!", 241, 4);
			}
		}
		if (pnp.npcType == 538) {
			p.frames.showChatboxInterface(p, 241);
		...
		if (pnp.npcType == 4906) {
			if (Engine.playerItems.HasItemAmount(p, 1511, 1) == true) {
				...
				p.frames.setString(p, "What do you want to talk about?", 228, 1);
				p.frames.setString(p, "Skillcape", 228, 2);
				p.frames.setString(p, "Legends Quest", 228, 3);
