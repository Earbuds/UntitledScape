...
public class SwitchItems2 implements Packet {
	...
	public void handlePacket(Player p, int packetId, int packetSize) {
		...
		if ((fromInterface == 50003968 || toInterface == 50003968) && fromInterface != toInterface) {
			...
			// "You can't drag a item from your inventory to your bank!");
			// } else {
			// p.frames.sendMessage(p,
			// "You can't drag a item from your bank to your inventory!");
			// }
			return;
		}
		switch (interfaceId) {
		case 762:
			...
			case 59: // Dragging an directly into tab: 9
				...
				break;
			default:
				// Shouldnt happen unless I missed something or player is using
				// a cheat client
				// Engine.fileManager.appendData("cheaters/"+p.username+".txt",
				// "Suspicious bank tab activities: "+tabId);
