...
public class Woodcutting {
	...
	public void startCutting(int objectid) {
		...
		if(getLevelForAxe(axeid) > getWoodcuttingLevel()) {
			p.frames.sendMessage(p, "You need level " + getLevelForAxe(axeid) + " to use this axe.");
			return;
		}
		
		p.frames.sendMessage(p, "You swing your axe at the tree.");
		
		maxlogs = rand.nextInt((10 - 1) + 1) + 1;
	...
	public void doLog() {
		...
		if (pit.freeSlotCount(p) < 1) {
			resetWoodcutting();
			p.frames.sendMessage(p, "Not enough inventory space to cut any more logs!");
		}
	...
	public int getLogsForTree(int tree) {
		...
		case 4:// Teak // TODO add to firemaking
			return 6333;

		case 5:// Maple
			return 1517;

		case 6:// Mahogany // TODO addto firemaking
			return 6332;
