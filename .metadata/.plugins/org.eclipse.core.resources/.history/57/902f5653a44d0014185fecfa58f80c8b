package untitledscape.players.combat;
...
public class PlayerCombat {
	...
	public void attackPlayer(Player p) {
		...
		if (p2 != null && p != null) {
			...
			if (p != null & p2 != null) {
				...
				int CombatXPRate = 50;
				if (p.attackPlayer <= 0 || p.isDead || p.attackPlayer >= Engine.players.length || Engine.players[p.attackPlayer] == null || Engine.players[p.attackPlayer].isDead || p2.disconnected[1]) {
					resetAttack(p);
				}
				if (p.combatDelay > 0) {
					return;
				...
				if (p.canAttackPlayer(p)) {
					...
					if (Misc.getDistance(p2.absX, p2.absY, p.absX, p.absY) >= 1 && UsingABow(p.equipment[3])) {
						...
						if (p.equipment[3] == 4214) {
							...
							p.frames.createGlobalProjectile(p.absY, p.absX, offsetY, offsetX, 249, 20, 31, 70, p2.playerId);
					...
					} else if (Misc.getDistance(p2.absX, p2.absY, p.absX, p.absY) <= 1 && UsingABow(p.equipment[3]) == false) {
						if (!p.usingSpecial) {
							p.requestAnim(p.attackEmote, 0);
						} else if (p.usingSpecial) {
							p.frames.setConfig(p, 301, 0);
							if (p.equipment[3] == 7806 && p.specialAmount >= 50) { // Anger
																					// Weapons
																					// Sword
							...
							if (p.equipment[3] == 7807 && p.specialAmount >= 100) { // Anger
																					// Weapons
																					// BAxe
								hitDamage = (60 + Misc.random(69));
								p.usingSpecial = false;
							...
							if (p.equipment[3] == 7808 && p.specialAmount >= 75) { // Anger
																					// Weapons
																					// Mace
								hitDamage = (60 + Misc.random(69));
								p.usingSpecial = false;
								...
								p.requestAnim(1978, 0);
								p.requestGFX(1222, 0);
							} else if (p.equipment[3] == 11730 && p.specialAmount >= 50) { // Saradomin
																							// sword
																							// by
																							// 1337
																							// pow3r
							...
							if (p.equipment[3] == 7809 && p.specialAmount >= 50) { // Anger
																					// Weapons
																					// Spear
								hitDamage = (60 + Misc.random(69));
								p.usingSpecial = false;
							...
							if (p.equipment[3] == 11694 && p.specialAmount >= 50) { // Armadyl
																					// godsword.
								...
								p.requestGFX(1222, 0);
							} else if (p.equipment[3] == 11696 && p.specialAmount == 100) { // Bandos
																							// godsword.
								hitDamage = Misc.random((int) (maxMeleeHit(p) * 1.2));
								p.usingSpecial = false;
								p.specialAmount -= 100;
								p.requestAnim(7073, 0);
								p.requestGFX(1223, 0);
							} else if (p.equipment[3] == 11698 && p.specialAmount >= 75) { // Saradomin
																							// godsword.
								hitDamage = Misc.random((int) (maxMeleeHit(p) * 1.3));
								p.usingSpecial = false;
								p.specialAmount -= 75;
								p.requestAnim(7071, 0);
								p.requestGFX(1220, 0);
							} else if (p.equipment[3] == 11700 && p.specialAmount >= 75) { // Zamorak
																							// godsword.
								hitDamage = Misc.random((int) (maxMeleeHit(p) * 1.3));
								p.usingSpecial = false;
								p.specialAmount -= 50;
								...
								p.requestGFX(1221, 0);
							} else if (p.equipment[3] == 4151 && p.specialAmount >= 50) { // Abyssal
																							// whip.
							...
							} else if (p.equipment[3] == 1305 && p.specialAmount >= 25) { // Dragon
																							// longsword.
								...
								p.requestAnim(1658, 0);
							} else if (p.equipment[3] == 4587 && p.specialAmount >= 70) { // Dragon
																							// scimitar.
								hitDamage = Misc.random((int) (maxMeleeHit(p) * 1.3));
								p.usingSpecial = false;
								p.specialAmount -= 70;
							...
							} else if (p.equipment[3] == 11730 && p.specialAmount >= 10) {// saradomin
																							// sword
								...
								p.requestAnim(7072, 0);
								p.requestGFX(1224, 100);
							} else if (p.equipment[3] == 1434 && p.specialAmount >= 40) { // Dragon
																							// mace.
								hitDamage = Misc.random((int) (maxMeleeHit(p) * 1.1));
								p.usingSpecial = false;
								p.specialAmount -= 40;
							...
							} else if (p.equipment[3] == 3204 && p.specialAmount >= 100) { // Dragon
																							// halberd.
							...
							} else if (p.equipment[3] == 5698 && p.specialAmount >= 25) { // Dragon
																							// dagger(s).
								p.usingSpecial = false;
								p.specialAmount -= 25;
						...
						// "Your Steel Titan joins the fight!");
						// p.frames.sendMessage(p7,
						// "Your opponents Steel Titan joins the fight!");
						// }
					...
					// p.frames.sendMessage(p,
					// "This player is not in the wild.");
					resetAttack(p);
				} else {
					resetAttack(p);
				}
	...
	public static int Bows[] = { 4212, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221, 4222, 4223, 837, 767, 4734, 839, 841, 843, 845, 847, 849, 851, 853, 855, 857, 859, 861, 2883, 4827, 6724, 11235 };
