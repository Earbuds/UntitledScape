package untitledscape.world.items;
...
public class Items {
	...
	public void createGroundItem(int id, int amt, int x, int y, int height, String owner) {
		...
		if (id >= 9747 && id <= 9814 || id >= 9848 && id <= 9950 || id >= 12169 && id <= 12171)// Skill
																								// Capes
																								// don't
																								// show
																								// up
		...
		if (groundItems[index].itemDroppedBy.equals("")) {
			...
			frames.createGroundItem(engine.players[engine.getIdFromName(owner)], id, amt, x, y, height);
		}
	}
	 ...
	 * @param itemX
	 *            The x coordinate to look for the item.
	 * @param itemY
	 *            The y coordinate to look for the item.
	 * @param height
	 *            The height level to look for the item.
