...
public class Mining {
	...
	public void startCutting(int objectid) {
		if (getPlayerPickaxe() == -1) {
			p.frames.sendMessage(p,
					"You don't have a pickaxe wich you have the Mining level to use.");
			return;
		}
		...
		if (getLevelForOre(rockid) > getMiningLevel()) {
			p.frames.sendMessage(p, "You need a mining level of "
					+ getLevelForOre(rockid) + " to mine this ore.");
			return;
		}
		p.frames.sendMessage(p, "You swing your pickaxe at the rock.");
		// p.frames.sendMessage(p, "You will get " + maxores +
		// " ores from this ore");
	...
	public void doOre() {
		...
		if (pit.freeSlotCount(p) < 1) {
			resetMining();
			p.frames.sendMessage(p, "Not enough inventory space to mine ore!");
		}
		if (pit.freeSlotCount(p) > 0) {
			p.frames.sendMessage(p, "You get some ore.");
			giveItem(getOreForRock(rockid));
			giveMiningXP(getXpForOre(rockid) * p.skillLvl[14]);
			p.requestAnim(anim, 0);
		}
	}
	...
	public int getPlayerPickaxe() {
		if (hasItem(1265))
			return 1265;
		if (p.equipment[3] == 1265)
			return 1265;
		if (hasItem(1267))
			return 1267;
		if (p.equipment[3] == 1267)
			return 1267;
		if (hasItem(1269))
			return 1269;
		if (p.equipment[3] == 1269)
			return 1269;
		if (hasItem(1271))
			return 1271;
		if (p.equipment[3] == 1271)
			return 1271;
		if (hasItem(1273))
			return 1273;
		if (p.equipment[3] == 1273)
			return 1273;
		if (hasItem(1275))
			return 1275;
		if (p.equipment[3] == 1275)
			return 1275;
