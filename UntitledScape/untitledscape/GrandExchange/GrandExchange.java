...
// import untitledscape.world.geList;
		...
		case 18:
			for (Offer Offer : offerList) {
				...
				if (Offer.type == 0 && !Offer.aborted) {
					// p.frames.setItemSlot(p, Offer.slot, 995,
					// Offer.currentPrice);
				} else if (!Offer.aborted) {
					// p.frames.setItemSlot(p, Offer.slot, Offer.item,
					// Offer.currentPrice);
				}
			}
			p.currentSlot = 0;
		...
		for (Offer Offer : offerList) {
			...
			for (Offer Offer2 : offerList) {
				...
				if (Offer2.item == Offer.item && Offer2.price >= Offer.price) {
					// Player p2 = Engine.players[Engine.getIdFromName(Offer2.owner)]; // Server.engine.players[Server.engine.getIdFromName(Offer2.owner)]
					// Player p3 = Engine.players[Engine.getIdFromName(Offer.owner)]; // Server.engine.players[Server.engine.getIdFromName(Offer.owner)]
					// int id = Offer.item;
