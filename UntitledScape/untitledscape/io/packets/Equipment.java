...
public class Equipment implements Packet {
	...
	private String[] hats = { "helm", "hood", "coif", "Coif", "hat", "mitre",
			"partyhat", "Hat", "full helm (t)", "full helm (g)", "hat (t)",
			"hat (g)", "cav", "boater", "Feather headdress", "helmet", "mask",
			"Helm of neitiznot", "Mime mask", "Sleep", "sleep" };
	private String[] boots = { "boots", "Boots" };
	private String[] gloves = { "gloves", "gauntlets", "Gloves", "vambraces",
			"vamb", "bracers" };
	private String[] shields = { "kiteshield", "sq shield", "Toktz-ket",
			"books", "book", "kiteshield (t)", "kiteshield (g)",
			"kiteshield(h)", "defender", "shield", "Saradomin kite",
			"Spirit Shield" };
	private String[] amulets = { "amulet", "necklace", "stole", "Amulet of",
			"scarf", "Scarf" };
	private String[] arrows = { "arrow", "arrows", "arrow(p)", "arrow(+)",
			"arrow(s)", "bolt", "Bolt rack", "Opal bolts", "Dragon bolts" };
	private String[] rings = { "ring" };
	private String[] body = { "platebody", "chainbody", "Zamorak d'hide",
			"robetop", "leathertop", "platemail", "top", "brassard",
			"Robe top", "body", "platebody (t)", "platebody (g)", "body(g)",
			"Saradomin plate", "body_(g)", "chestplate", "torso", "shirt",
			"Varrock armour", "Guthix d'hide", "Saradomin d'hide",
			"Prince tunic", "Wizard robe (g)", "Wizard robe (t)",
			"Runecrafter robe" };
	private String[] legs = { "platelegs", "knight robe", "plateskirt",
			"skirt", "bottoms", "chaps", "platelegs (t)", "platelegs (g)",
			"bottom", "skirt", "skirt (g)", "skirt (t)", "chaps (g)",
			"chaps (t)", "tassets", "legs", "Saradomin legs", "trousers" };
	private String[] weapons = { "scimitar", "Rubber chicken", "longsword",
			"Swords", "Longsword", "sword", "crozier", "longbow", "shortbow",
			"dagger", "mace", "halberd", "spear", "Abyssal whip", "axe",
			"flail", "crossbow", "Torags hammers", "dagger(p)", "dagger(+)",
			"dagger(s)", "spear(p)", "kite", "spear(+)", "spear(s)",
			...
			"godsword", "c'bow", "Crystal bow", "Dark bow",
			"Magic butterfly net", "claws", "banner", "Invisibility", "claws",
			"Claws", "Warhammer" };

	/* Fullbody is an item that covers your arms. */
	private String[] fullbody = { "top", "shirt", "platebody", "Platebody",
			"Zamorak d'hide", "Ahrims robetop", "Karils leathertop",
			"brassard", "Robe top", "robetop", "platebody (t)",
			"platebody (g)", "chestplate", "torso", "chainbody",
			"Varrock armour", "Guthix d'hide", "Saradomin d'hide",
			"Prince tunic", "Wizard robe (g)", "Wizard robe (t)",
			"Runecrafter robe" };

	/* Fullhat covers your head but not your beard. */
	private String[] fullhat = { "med helm", "Dharoks helm", "hood",
			"Initiate helm", "Coif", "Helm of neitiznot" };

	/* Fullmask covers your entire head. */
	private String[] fullmask = { "full helm", "Slayer helmet", "Veracs helm",
			"Guthans helm", "Torags helm", "Karils coif", "full helm (t)",
			"full helm (g)", "Green h'ween mask", "Red h'ween mask",
			"Blue h'ween mask", "full helmet" };
	...
	public void handlePacket(Player p, int packetId, int packetSize) {
		...
		if (p.items[index] == wearId) {
			...
			if (p.equipment[3] == 4037 && p.AtCastleWars()
					|| p.equipment[3] == 4039 && p.AtCastleWars()) {
				return;
			}
			...
			if (wearId == 3101) {
				if ((p.rights < 1) && (p.member < 1)) {
					pi.deleteItem(p, wearId, pi.getItemSlot(p, wearId), 100);
					p.frames.sendMessage(p,
							"You must become a member before you can wear this!");
					return;
				}
			}
			if ((wearId == 4566) || (wearId == 5554) || (wearId == 1361)
					|| (wearId == 5554) || (wearId == 5555) || (wearId == 5556)
					|| (wearId == 5557) || (wearId == 2653) || (wearId == 2655)
					|| (wearId == 2657) || (wearId == 2659) || (wearId == 2661)
					|| (wearId == 2663) || (wearId == 2665) || (wearId == 2667)
					|| (wearId == 2665) || (wearId == 2667) || (wearId == 2669)
					|| (wearId == 2671) || (wearId == 3488) || (wearId == 3486)
					|| (wearId == 2673) || (wearId == 2675) || (wearId == 3481)
					|| (wearId == 3483) || (wearId == 534) || (wearId == 12222)) {
				if ((p.rights < 1) && (p.member < 1)) {
			...
			for (int i = 0; i < 24; i++) {
				if (Misc.getSkillName(i) != null) {
					if (Engine.items.getItemName(wearId).toLowerCase()
							.contains(Misc.getSkillName(i).toLowerCase())
							&& Engine.items.getItemName(wearId).toLowerCase()
									.contains("cape")
							&& p.getLevelForXP(i) < 99) {
						p.frames.sendMessage(p, "You are not level 99 in "
								+ Misc.getSkillName(i) + "!");
						return;
					}
				}
				if (Engine.items.getItemName(wearId).toLowerCase()
						.contains("woodcut")
						&& Engine.items.getItemName(wearId).toLowerCase()
								.contains("cape") && p.getLevelForXP(8) < 99) {
					p.frames.sendMessage(p,
							"You are not level 99 in Woodcutting!");
					return;
				}
				if (Engine.items.getItemName(wearId).toLowerCase()
						.contains("constru")
						&& Engine.items.getItemName(wearId).toLowerCase()
								.contains("cape") && p.getLevelForXP(22) < 99) {
					p.frames.sendMessage(p,
							"You are not level 99 in Construction!");
					return;
				}
				if (Engine.items.getItemName(wearId).toLowerCase()
						.contains("runecraft")
						&& Engine.items.getItemName(wearId).toLowerCase()
								.contains("cape") && p.getLevelForXP(20) < 99) {
					p.frames.sendMessage(p,
							"You are not level 99 in RuneCrafting!");
					return;
				}
			}

			if (wearId == 6110 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 6110, pi.getItemSlot(p, 6110), 28);
				return;
			}
			if (wearId == 6111 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 6111, pi.getItemSlot(p, 6111), 28);
				return;
			}
			if (wearId == 6106 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 6106, pi.getItemSlot(p, 6106), 28);
				return;
			}
			if (wearId == 6107 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 6107, pi.getItemSlot(p, 6107), 28);
				return;
			}
			if (wearId == 6108 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 6108, pi.getItemSlot(p, 6108), 28);
				return;
			}
			if (wearId == 6109 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 6109, pi.getItemSlot(p, 6109), 28);
				return;
			}

			if (wearId == 7806 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 7806, pi.getItemSlot(p, 7806), 28);
				return;
			}
			if (wearId == 7807 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 7807, pi.getItemSlot(p, 7807), 28);
				return;
			}
			if (wearId == 7808 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 7808, pi.getItemSlot(p, 7808), 28);
				return;
			}
			if (wearId == 7809 && p.rights <= 0) {
				p.frames.sendMessage(p,
						"You must become a staff member before you can wear this.");
				pi.deleteItem(p, 7809, pi.getItemSlot(p, 7809), 28);
				return;
			}

			if (wearId == 3140 && p.DragonSlayer != 5 || wearId == 1127
					&& p.DragonSlayer != 5) {
				p.frames.sendMessage(p,
						"You must complete dragon slayer before you can wear this.");
				return;
			}
			if (wearId == 9813 && p.QuestPoints < 2 || wearId == 9814
					&& p.QuestPoints < 2) {
				p.frames.sendMessage(p,
						"You must complete all of New Rune's quests to wear this.");
				return;
			}

			if (cLAttack > p.getLevelForXP(0)) {
				p.frames.sendMessage(p, "You need " + cLAttack
						+ " Attack to equip this item.");
				return;
			}
			if (cLDefence > p.getLevelForXP(1)) {
				p.frames.sendMessage(p, "You need " + cLDefence
						+ " Defence to equip this item.");
				return;
			}
			if (cLCrafting > p.getLevelForXP(20)) {
				p.frames.sendMessage(p, "You need " + cLCrafting
						+ " Runecrafting to equip this item.");
				return;
			}
			if (cLStrength > p.getLevelForXP(2)) {
				p.frames.sendMessage(p, "You need " + cLStrength
						+ " Strength to equip this item.");
				return;
			}
			if (cLMagic > p.getLevelForXP(6)) {
				p.frames.sendMessage(p, "You need " + cLMagic
						+ " Magic to equip this item.");
				return;
			}
			if (cLRanged > p.getLevelForXP(4)) {
				p.frames.sendMessage(p, "You need " + cLRanged
						+ " Ranged to equip this item.");
				return;
			}
			...
			if (wearId >= 0 && wearAmt > 0) {
				...
				if (p.equipment[targetSlot] >= 0
						&& (wearId != p.equipment[targetSlot] || !Engine.items
								.stackable(wearId))) {
	...
	public int getCLRanged(int ItemID) {
		...
		if (itemName.toLowerCase().contains("3rd age range")
				|| itemName.toLowerCase().contains("3rd age vam")) {
			return 75;
		}
	...
	public int getCLMagic(int ItemID) {
		...
		if (itemName.toLowerCase().contains("3rd age robe")
				|| itemName.toLowerCase().contains("3rd age mage")) {
			return 75;
		}
	...
	public int getCLAttack(int ItemID) {
		...
		if (itemName.equals("Veracs flail") || ItemID == 1337 || ItemID == 1203
				|| ItemID == 1231 || ItemID == 3101) {
			return 70;
		}
		if (itemName.equals("Torags hammers")) {
			return 70;
	...
	public int getCLDefence(int ItemID) {
		...
		if (ItemID == 8798 || ItemID == 8799 || ItemID == 8800
				|| ItemID == 8802 || ItemID == 8803) {
			return 30;
		}
		if (ItemID == 7990) {
			return 60;
	 ...
	 * 
	 * @param item
	 *            The item id to get the type for.
