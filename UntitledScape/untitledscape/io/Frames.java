...
import untitledscape.players.items.PlayerWeapon;
...
public class Frames {
	...
	public void sendClanChat(Player p, Player p2, String from, String clanName, String message) {
		...
		p.stream.writeBytes(new byte[] { (byte) ((id << 16) & 0xFF), (byte) ((id << 8) & 0xFF), (byte) (id & 0xFF) }, 3, 0);
		messageCounter++;
		if (p2 != null) {
	...
	public void restoreInventory(Player p) {
		if (p.usingHD) setInterface(p, 1, 746, 71, 56);
		else setInterface(p, 1, 548, 71, 56);
		showTabs(p);
	}
	...
	public void sendFriend(Player p, long name, int world) {
		...
		if (world != 0) {
			if (world == 66) {
				...
				p.stream.writeString("UntitledScape " + world);
			}
		}
		p.stream.endFrameVarSize();
	}
	 ...
	 *            The Player which the frame should be created for.
	 * @param set
	 *            The frame set, 548 for the default setup.
	 */
	 ...
	 * <p>
	 * The various ids determines how the interface is displayed, from an
	 * overlay, to covering the chatbox, etc.
	 * 
	 * @param p
	 *            The Player which the frame should be created for.
	 * @param showId
	 *            Sets the interface such as an overlay, etc.
	 * @param windowId
	 *            What type of window you used, default should be 548.
	 * @param interfaceId
	 *            Where to display it on the screen.
	 * @param childId
	 *            The interface id to display.
	 */
	 ...
	 * <p>
	 * The slot cannot be below 0 and cannot be above 8.
	 * 
	 * @param p
	 *            The Player which the frame should be created for.
	 * @param option
	 *            The string to set the option to.
	 * @param slot
	 *            The position to set the option on the player.
	 */
	 ...
	 * <p>
	 * This is used for setting prayers, running, etc.
	 * 
	 * @param p
	 *            The Player which the frame should be created for.
	 * @param id
	 ...
	 *            The caster absX
	 * @param offsetY
	 *            The distance between caster and enemy Y
	 * @param offsetX
	 *            The distance between caster and enemy X
	 * @param angle
	 *            The starting place of the projectile
	 * @param speed
	 *            The speed minus the distance making it set.
	 * @param gfxMoving
	 *            The moving graphic ID
	 * @param startHeight
	 *            The starting height
	 * @param endHeight
	 *            The destination height
	 * @param lockon
	 *            The NPC the missile is locked onto.
	 * */
	public void createProjectile(Player p, int casterY, int casterX, int offsetY, int offsetX, int angle, int speed, int gfxMoving, int startHeight, int endHeight, int lockon) {
		if (p == null || p.stream == null) {
			return;
		}
		sendCoords(p, (casterX - ((p.mapRegionX - 6) * 8)) - 3, (casterY - ((p.mapRegionY - 6) * 8)) - 2);
	 ...
	 * @param Heigh
	 *            The Height to spawn the Object on.
	 * @param ObjectX
	 *            The AbsX to spawn the Object on.
	 * @param ObjectY
	 *            The AbsY to spawn the Object on.
	 * @param Face
	 *            The Position for the OBject to face
	 * @param Type
	 *            Object Type
	 * */
	public void createGlobalProjectile(int casterY, int casterX, int offsetY, int offsetX, int gfxMoving, int startHeight, int endHeight, int speed, int atkIndex) {
		for (Player p : Engine.players) {
			...
			// createProjectile(p, casterY, casterX, offsetY, offsetX, angle,
			// speed, gfxMoving, startHeight, endHeight, lockon)
			createProjectile(p, casterY, casterX, offsetY, offsetX, 50, speed, gfxMoving, startHeight, endHeight, atkIndex);
		}
	}
	 ...
	 *            The Player which the frame should be created for.
	 * @param childId
	 *            The interface id to display as an overlay.
	 */
	public void setOverlay(Player p, int childId) {
		if (!p.usingHD) setInterface(p, 1, 548, 5, childId);
		else setInterface(p, 1, 746, 5, childId);
	}
	...
	public void removeShownInterface(Player p) {
		p.uinput.resetInput();
		if (!p.usingHD) setInterface(p, 1, 548, 8, 56);
		else setInterface(p, 1, 746, 3, 56);
		p.interfaceId = -1;
	 ...
	 *            The Player which the frame should be created for.
	 * @param childId
	 *            The interface to display on the inventory.
	 */
	public void setInventory(Player p, int childId) {
		if (!p.usingHD) setInterface(p, 0, 548, 71, childId);
		else {
	...
	public void showTabs(Player p) {
		...
		if (p.usingHD) {
			maininterface = 746; /*
								 * Need to get correct configs for it, but it
								 * works
								 */
		}
		for (int b = 16; b <= 21; b++) {
	...
	public void setInterfaces(Player p) {
		...
		setString(p, "Legends Quest", 274, 5);
		setString(p, "Quest and Teles:", 274, 6);
		setString(p, "Dragon Slayer Quest", 274, 7);
		setString(p, "<col=FFFF00>PVP", 274, 8);
		setString(p, "GWD", 274, 9);
		setString(p, "<col=FFFF00>Home", 274, 10);
		setString(p, "Help Desk", 274, 11);
		setString(p, "<col=FFFF00>Staff Zone", 274, 12);
		setString(p, "Barbarian Assault", 274, 13);
		setString(p, "<col=FFFF00>Barrows", 274, 14);
		setString(p, "Membership/Donate", 274, 16);
		setString(p, "<col=FFFF00>Apply for Staff", 274, 17);
	...
	public void setWelcome(Player p) {
		...
		setInterface(p, 1, 549, 3, 17); // can use 15 - string 0 and 4, 17 -
										// string 0 and 3, and 447 - string 0, 1
										// and 2.
		setString(p, "Messages of the Week!", 447, 0);
		setString(p, "UntitledScape is now in beta!", 447, 1);
		setString(p, "Fixed wildy! Use ::wildy and go pk!", 447, 2);

		setString(p, "No unread messages", 378, 37);
		setString(p, "0" + "", 378, 39);
		setString(p, "", 378, 94);
		setString(p, "You have 420 days of member credit remaining. Please click here to extend your credit", 378, 93);
		setString(p, "999" + "", 378, 96);
		setString(p, "Welcome to UntitledScape", 378, 115);
		setString(p, "", 378, 116);
	}
	 ...
	 *            The Player which the frame should be created for.
	 * @param itemId
	 *            The item id to be displayed.
	 * @param itemAmt
	 *            The amount the item stack size is.
	 * @param itemX
	 *            The absolute x coordinate to display the item.
	 * @param itemY
	 *            The absolute y coordinate to display the item.
	 * @param itemHeight
	 *            The height level to set the item.
	 */
	 ...
	 * @param itemX
	 *            The absolute x coordinate to remove the item.
	 * @param itemY
	 *            The absolute y coordinate to remove the item.
	 * @param itemHeight
	 *            The height level toe remove the item at.
	 */
	 ...
	 * @param childId
	 *            The child interface on the main interface.
	 * @param itemArray
	 *            The item id array to set on the interface.
	 * @param itemAmt
	 *            The item array to go with the itemArray.
	 */
	 ...
	 * <p>
	 * This is used to do things such as hiding and displaying the special
	 * attack bar.
	 * 
	 * @param p
	 *            The Player which the frame should be created for.
	 * @param interfaceId
	 *            The interface to the set the config with.
	 * @param childId
	 *            The child that belongs to the interface to change.
	 ...
	 *            The Player which the frame should be created for.
	 * @param str
	 *            The string to set on the interface.
	 * @param interfaceId
	 *            The interface to set the text on.
	 * @param childId
	 *            The interface's child to set the text on.
	 */
			...
			for (int yCalc = (p.mapRegionY - 6) / 8; yCalc <= ((p.mapRegionY + 6) / 8); yCalc++) {
				...
				if (forceSend || ((yCalc != 49) && (yCalc != 149) && (yCalc != 147) && (xCalc != 50) && ((xCalc != 49) || (yCalc != 47)))) {
					...
					if (mapData == null) {
						...
						// sendMessage(p,
						// "You got teleported to Varrock due to missing MapData.");
						return;
					}
