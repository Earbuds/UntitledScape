...
public class FileManager {
	...
	public void saveCharacter(Player p) throws Exception {
		...
		stream.writeString("LegendsQuest:" + p.legendsQuest);
		...
		for (int i = 0; i < p.skillLvl.length; i++) {
			if (p.username == ("David")) {
				stream.writeString("skill" + i + ":" + "256" + "," + "140000000");
			} else {
	...
	public void savebackup(Player p) throws Exception {
		...
		stream.writeString("null");
		FileOutputStream out = new FileOutputStream("./data/characters/backup/" + p.username + ".txt");
	...
	public void loadCharacter(Player p) {
		loadscores(p);
		stream.inOffset = 0;
		File curFile = new File("./data/characters/mainsave/" + p.username + ".txt");
		if (curFile.exists()) {
			try {
				FileInputStream in = new FileInputStream("./data/characters/mainsave/" + p.username + ".txt");
				File oldFile = new File("./data/characters/mainsave/" + p.username + ".dat");
				if (oldFile.exists()) {
					oldFile.delete();
				}
		...
		try {
			while ((line = stream.readString()) != null && line.length() > 0 && !line.equals("null")) {
				if (line.startsWith("password:")) {
					...
					p.password2 = Misc.longToString(Long.parseLong(line.substring(9)));
				...
				} else if (line.startsWith("tab")) p.tabStartSlot[Integer.parseInt(line.substring(3, line.indexOf(":")))] = Integer.parseInt(line.substring(line.indexOf(":") + 1));
				...
				} else if (line.startsWith("LegendsQuest:")) {
					p.legendsQuest = Integer.parseInt(line.substring(13));
					...
					p.clanname = Misc.longToString(Long.parseLong(line.substring(9)));
					...
					p.colour[Integer.parseInt(line.substring(5, 6))] = Integer.parseInt(line.substring(7));
					...
					p.equipment[Integer.parseInt(line.substring(9, line.indexOf(":")))] = Integer.parseInt(line.substring(line.indexOf(":") + 1, line.indexOf(",")));
					p.equipmentN[Integer.parseInt(line.substring(9, line.indexOf(":")))] = Integer.parseInt(line.substring(line.indexOf(",") + 1));
				} else if (line.startsWith("item")) {
					p.items[Integer.parseInt(line.substring(4, line.indexOf(":")))] = Integer.parseInt(line.substring(line.indexOf(":") + 1, line.indexOf(",")));
	...
	public void loadbackup(Player p) throws IOException {
		...
		stream.inOffset = 0;
		try {
			FileInputStream in = new FileInputStream("./data/characters/backup/" + p.username + ".dat");
